...
import { fetchLibAlbums } from './helpers/thunk';
...
const LibAlbums = () => {
    ...
    const { reload } = useSelector(state => state.libAlbumsSlice)
    ...
    const {
        albums
    } = useSelector(state => state.libAlbumsSlice)
    
    return (
        <section>
            <div className='section-name'>
                <h1>Збережені альбоми</h1>
